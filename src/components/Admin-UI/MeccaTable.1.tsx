...
import AddNewMeccaModal from "./AddNewMainModal";

export const MeccaTable = ({ isReversed }: { isReversed: boolean }) => {
  ...
  const handleDropSub = async (e: any, target: string) => {
    ...
    const droppedSubId = JSON.parse(droppedData)?.sub_id;
    if (target === "trash") {
      console.log("ドロップされた要素のsub_id:", droppedSubId.slice(0, 5));
      await deleteSub(droppedSubId);
      const hideSubInput = document.querySelector(
        `.a${droppedSubId.slice(0, 5)}`
      );
      hideSubInput?.setAttribute("disabled", "disabled");
      const hideSubDiv = document.querySelector(
        `.b${droppedSubId.slice(0, 5)}`
      );
  ...
  const handleDropMain = async (e: any, target: string) => {
    e.preventDefault();
    const droppedData = e.dataTransfer.getData("application/json");
    const droppedMainId = JSON.parse(droppedData)?.sub_id;
    if (target === "trash") {
      console.log("ドロップされた要素のsub_id:", droppedMainId.slice(0, 5));
      await deleteMain(droppedMainId);
      const hideMain = document.querySelector(`.${droppedMainId.slice(0, 5)}`);
      hideMain?.setAttribute("disabled", "disabled");
      handleRefresh();
    }
  };
            ...
            <label className="form-control w-full max-w-xs">
              <div className="label">
                {!isMainModalOpen ? (
                  ...
                  <div className="relative w-full">
                    <AddNewMeccaModal
                      closeModal={handleOpenMainModal}
              ...
              {selectedAnime?.main.map((mainPlaces: any, index: number) => (
                <input
                  type="text"
                  placeholder="Type here"
                  className="input input-bordered w-full max-w-xs mb-1.5"
